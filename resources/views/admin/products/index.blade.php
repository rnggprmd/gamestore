...
<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-6">
    <div class="flex items-center justify-between">
        <h3 class="text-base font-bold text-slate-800 dark:text-white flex items-center gap-2">
            <i data-lucide="package" class="w-4.5 h-4.5 text-indigo-500"></i>
            Daftar Produk ({{ count($products) }})
        </h3>
        <a href="{{ route('admin.products.create') }}" class="inline-flex items-center gap-1.5 bg-violet-600 hover:bg-violet-500 text-white text-xs font-bold px-4 py-2.5 rounded-xl transition-all shadow-lg shadow-violet-500/10 cursor-pointer">
                ...
                @forelse($products as $product)
                    <tr class="text-slate-600 dark:text-slate-300 hover:bg-slate-50/50 dark:hover:bg-slate-800/10">
                        <td class="py-4 px-4 font-semibold text-slate-800 dark:text-slate-200">
                            {{ $product->game->name ?? 'Game Tidak Ditemukan' }}
                        </td>
                        <td class="py-4 px-4 font-medium text-slate-700 dark:text-slate-300">{{ $product->name }}</td>
                        <td class="py-4 px-4 text-xs">
                            <span class="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-850 text-slate-500 font-medium">
                                {{ $product->category->name ?? '-' }}
                            </span>
                        </td>
                        <td class="py-4 px-4 text-right font-bold text-violet-500">
                        ...
                        <td class="py-4 px-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.products.edit', $product->id) }}" class="p-2 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-violet-600 hover:text-white transition-all text-slate-600 dark:text-slate-300">
